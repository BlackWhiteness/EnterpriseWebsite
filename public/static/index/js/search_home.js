...
var area = '', city = '', measurearea = '', floor_type = '', mianji = '', title = '';
var is_sale = '';
...
//处理点击事件
$("#measurearea a").click(function () {
    var position = $("#measurearea a").index(this);
    $("#measurearea a").attr("class", "");
    $("#measurearea a").eq(position).attr("class", 'current');
    measurearea = $("#measurearea a").eq(position).attr('value');
    action();
});
...
$("#floor_type a").click(function () {
    var position = $("#floor_type a").index(this);
    $("#floor_type a").attr("class", "");
    $("#floor_type a").eq(position).attr("class", 'current');
    floor_type = $("#floor_type a").eq(position).attr('value');
    action();
});
...
$("#struck a").click(function () {
    let position = $("#struck a").index(this);
    $("#struck a").attr("class", "");
...
function resultFilter(result) {
    $.each(result.data, function (index, row) {
        ...
        html += "<a href=/index/private_home/detail?id=" + row.id + " target='_blank' title=" + row.title + ">";
        ...
        html += "<a href=/index/private_home/detail?id=" + row.id +  " target='_blank'>";
        ...
        html += "<a class='cf_ckxq' href=/index/private_home/detail?id=" + row.id + " target='_blank'>查看详情</a>" + "</div></div>"
