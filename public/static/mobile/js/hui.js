...
var hui = (function(selector){
	...
	var hcExtends = {
		...
		unfold : function(height, text){
			if(this.length < 1){return} if(!height){height = 288;} if(!text){text = '';}
			// this.css({height:height+'px'});
            this.css({height:'4.1rem'});
			...
			hui('#hui-unfold').click(function(){
				if(hui(this).hasClass('huiRotate')){
                    hui(this).parent().css({height:'4.1rem'});
                    hui(this).css({position:'absolute'});
                    hui(this).removeClass('huiRotate');
				}else{
                    hui(this).parent().css({height:'auto'});
                    hui(this).css({position:'initial'});
                    hui(this).addClass('huiRotate');
				}
			});
		}
	}
	...
	_hui.showSlideMenu = function(){
	    ...
	    if(header.length > 0){
	    	...
	    	menu.css({top : (sets.top + 0 + hui.immersedStatusbarHeight) +'px'});
	    }
