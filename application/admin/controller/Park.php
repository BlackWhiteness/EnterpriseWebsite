...
class Park extends Adminbase
{
    ...
    public function index(Request $request)
    {
        if ($this->request->isAjax()) {
            $result = ParkManage::order(array('id' => 'DESC'))
                ->paginate($request->param('limit'));
            $total = $result->total();
            $result = array("code" => 0, "count" => $total,
                "data" => ParkFormat::getInstance()->formatList($result->items()));
            return json($result);
        }
        return $this->fetch();
    }
    ...
    public function edit(Request $request)
    {
        if ($request->isPost()) {
            ...
            $rs = ParkManage::where(["id" => $id])->find();
            $this->assign("data", $rs);
            $result = $this->City_Model->order(array('id' => 'DESC'))->select()->toArray();
            $citys = '';
            foreach ($result as $r) {
