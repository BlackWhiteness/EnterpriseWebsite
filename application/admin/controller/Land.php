...
class Land extends Adminbase
{
    ...
    public function index(Request $request)
    {
        if ($this->request->isAjax()) {
            $result = LandManage::order(array('id' => 'DESC'))
                ->paginate($request->param('limit'));
            $total = $result->total();
            $result = array("code" => 0, "count" => $total,
                "data" => LandFormat::getInstance()->formatList($result->items()));
            return json($result);
        }
        return $this->fetch();
    }
    ...
    public function edit(Request $request)
    {
        if ($request->isPost()) {
            ...
            $rs = LandManage::where(["id" => $id])->find();
            $this->assign("data", $rs);
            $result = $this->City_Model->order(array('id' => 'DESC'))->select()->toArray();
            $citys = '';
            foreach ($result as $r) {
    ...
    public function listorder()
    {
        ...
        $rs = LandManage::update(['id' => $id, 'listorder' => $listorder]);
        if ($rs) {
            $this->success("菜单排序成功！");
        } else {
            $this->error("菜单排序失败！");
    ...
    public function setstate()
    {
        ...
        if (LandManage::update(['status' => $status], ['id' => $id])) {
            $this->success('操作成功！');
        } else {
            $this->error('操作失败！');
        }
