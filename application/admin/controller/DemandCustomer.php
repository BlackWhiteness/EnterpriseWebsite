...
class DemandCustomer extends Adminbase
{
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            $result = CustomerDemand::order(array('id' => 'DESC'))->select()->toArray();
            $total = count($result);
    ...
    public function edit()
    {
        if ($this->request->isPost()) {
            ...
            $rs = CustomerDemand::where(["id" => $id])->find();
    ...
    public function listorder()
    {
        ...
        $rs = CustomerDemand::update(['id' => $id, 'listorder' => $listorder]);
        if ($rs) {
            $this->success("菜单排序成功！");
        } else {
            $this->error("菜单排序失败！");
    ...
    public function setstate()
    {
        ...
        if (CustomerDemand::update(['status' => $status], ['id' => $id])) {
            $this->success('操作成功！');
        } else {
            $this->error('操作失败！');
        }
